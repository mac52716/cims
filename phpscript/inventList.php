...
if (!empty($_REQUEST['searchVal'])) {
	...
	$sql = "SELECT box_no,package,lf_name,machine_model,dra,clamp_serno,insert_serno,in_out,out_count,status,shots,max_shots,machine_no,remarks,date_time FROM inventory WHERE lf_name LIKE '%".$searchVal."%' or
	...
	$sql = "SELECT box_no,package,lf_name,machine_model,dra,clamp_serno,insert_serno,in_out,out_count,status,shots,max_shots,machine_no,remarks,date_time FROM inventory ORDER BY date_time DESC";
	$r_query = mysqli_query($conn,$sql);
